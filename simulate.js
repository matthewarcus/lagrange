...
Lagrange.simulate = function(canvas,system) {
    ...
    function run1(system,t,dt) {
        while (dt > 0) {
            ...
            Lagrange.step(f,p,v,t,delta/1000,eps);
            dt -= delta
        }
    ...
    var lasttime = 0;
    var xs = [], ys = [];
    var start = 0, psize = 0, maxpsize = 150;

    function redisplay(timestamp) {
        if (lasttime == 0) {
            lasttime = timestamp;
        }
        ...
        ctx.beginPath(); ctx.arc(ox,oy,10,0,2*Math.PI); ctx.fill();
        ctx.beginPath(); ctx.arc(x1,y1,10,0,2*Math.PI); ctx.fill();
